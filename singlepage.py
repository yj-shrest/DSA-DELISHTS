...
class SingleFrame(CTkFrame):
    def __init__(self, parent_frame, recipe_data):
        ...
        title_frame.place(relx= 0.02, rely=0.48, relwidth =0.9, relheight= 0.3)
        title_label = CTkLabel(title_frame, text=recipe_data["recipe"].upper(), text_color="white",anchor=W, font=("Arial", 21,'bold'),width=100,height=30)
        ...
        timer_frame = CTkFrame(title_frame,bg_color="transparent",fg_color="transparent")
        ...
        start_button = CTkButton(timer_frame,hover=False,corner_radius=30,text="Start", text_color="white", fg_color="#2196F3", font=("Arial", 20,"bold"), height=30 , command=self.start_timer)
        start_button.pack(pady=20,side=RIGHT)
        clock_frame = CTkFrame(title_frame,bg_color="transparent",fg_color="transparent")
        clock_frame.pack(fill=X)
        self.label = CTkLabel(clock_frame, text="00:00:00",text_color="white",font=("Arial", 24))
    ...
    def start_timer(self):
        self.seconds = int(self.min.get())*60
        self.update_timer()
